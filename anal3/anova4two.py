...
# 교호작용(interaction term) : 한 쪽 요인이 취하는 수준에 따라 다른 쪽 요인이 영향을 받는 요인의 조합효과를 말하는 것으로 상승과 상쇄효과가 있음  
# ex) 초밥과 간장, 감자튀김과 간장, 초밥과 케찹 ... 
...
# 하지만 태아수와 관측자의 수의 상호작용은 유의하지 않음. 
...
# 모든 집단 별 표본 수가 동일하므로 균형 설계가 잘 되었다라고 할 수 있다. 
result2 = ols('time ~ C(poison) * C(treat)', data=data2).fit()
print(anova_lm(result2))

#                       df    sum_sq   mean_sq          F        PR(>F)
# C(poison)            2.0  1.033012  0.516506  23.221737  3.331440e-07 < 0.05 이므로 귀무기각 
# C(treat)             3.0  0.921206  0.307069  13.805582  3.777331e-06 < 0.05 이므로 귀무기각
# C(poison):C(treat)   6.0  0.250138  0.041690   1.874333  1.122506e-01 > 0.05 이므로 상호작용효과는 없다. 
