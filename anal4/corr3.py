...
def setScatterGraph(tour_table, all_table, tourPoint):
    # print(tourPoint)
    # 계산할 관광지명에 해당하는 자료만 뽑아 별도 저장하고, 외국인 관광 자료와 병합 
    ...
    plt.xlabel('중국인 입국수')
    plt.ylabel('외국인 입장객수')
    lamb1 = lambda p:merge_table['china'].corr(merge_table['ForNum'])   # lamda객체 생성 
    r1 = lamb1(merge_table)
    print('r1 : ', r1)
    plt.title('r={:.5f}'.format(r1))
    plt.scatter(merge_table['china'], merge_table['ForNum'], alpha=0.7, s=6, c='red') # alpha -> 투명도 조절
    ...
    return [tourPoint, r1, r2, r3]   # []가 없으면 튜플, 있으면 리스트 
