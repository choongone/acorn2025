...
print("각 행의 합", np.sum(a, axis=0))
print("각 행의 최대값", np.max(a, axis=0))


# 6행 6열의 다차원 zero 행렬 객체를 생성한 후 다음과 같이 indexing 하시오.

a = np.arange(1,37).reshape(6,6)
print(a)
print()

print("두번째 행 출력 결과: ", a[1])  
# a는 ndarray객체이므로 a[인덱스]처럼 접근해야됨 
# a[1]->두번째 행 
# a[3]->네번째 행 
print("다섯번째 열 출력 결과: ", a[:, 4])
# a[행, 열]
# 열만 건드리고 싶을때는 :를 넣으면 모든 행으로 인식 
print("출력 결과: ") 
print(a[2:5,2:5])
# 3~5행과 3~5열을 출력 
print()
...
a = np.zeros((6,4), dtype = int) # 6행 4열 다차원 zero 행렬 객체 생성 
for i in range(6):
    b = np.random.randint(20,101)
    a[i][0] = b
    for j in range(1,4):
        a[i][j] = b + j
print(a)
...
print(np.insert(a,0,1000,axis=0))
...
print('1사분위 수 :', np.percentile(arr, 25))  # percentile -> 데이터 분포를 확인하기 쉬움(뒤에 숫자는 퍼센트)
print('arr: ',arr)
aa = np.sort(arr)
print(aa)
lowerdata = arr[arr <= np.percentile(arr, 25)]
print('lowerdata:', lowerdata)
print('2사분위 수 :', np.percentile(arr, 50))  # percentile은 내부적으로 sort해줌
