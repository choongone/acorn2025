# Broadcasting : 크기가 다른 배열 간의 연산 시 배열의 구조 자동 변환
# 작은 배열과 큰 배열 연산 시 작은 배열은 큰 배열에 구조를 따름
...
# 두 배열의 요소 더하기
# 1) 새로운 배열을 이용
z = np.empty_like(x) # x와 구조가 같은 새로운 배열을 만듦
print(z) 
for i in range(3):
    z[i] = x[i] + y
print(z)
...
print('\n넘파이로 파일 i/o')
np.save('numpy4etc', x) # binary 형식으로 저장
