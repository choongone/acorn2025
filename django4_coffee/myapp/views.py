from django.shortcuts import render, redirect  # 시각화 할 때 저장경로로 
import os
...
def surveyProcess(request):
    ...
    rdata = list(Survey.objects.all().values())     # 데이터 프레임으로 만들기 위해 리스트로 만듦.
    # print(rdata)
...
def analysisFunc(rdata):
    ...
    return crossTbl, results, df     # return 값은 한개만 넣을 수 있음. 이 경우는 튜플이기 때문에 한개로 취급 
